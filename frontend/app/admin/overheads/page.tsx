...
const OVERHEAD_ROWS: OverheadRow[] = [
...
export default function AdminOverheadsPage() {
  ...
  const monthlyAverage = total ? total.annualTotal / 12 : 0;
  const annualDelta = total ? total.annualTotal - total.comparisonTotal : 0;
    ...
    <div>
      ...
      <div className="row section">
        <div className="card col" style={{ minWidth: 220 }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 700, marginTop: 6 }}>{formatCurrency(total?.annualTotal ?? 0)}</div>
        </div>
        <div className="card col" style={{ minWidth: 220 }}>
          <div className="subtle">Comparison total</div>
          <div style={{ fontSize: 26, fontWeight: 700, marginTop: 6 }}>{formatCurrency(total?.comparisonTotal ?? 0)}</div>
        </div>
        <div className="card col" style={{ minWidth: 220 }}>
          <div className="subtle">Annual delta</div>
          <div style={{ fontSize: 26, fontWeight: 700, marginTop: 6 }}>{formatCurrency(annualDelta)}</div>
        </div>
        <div className="card col" style={{ minWidth: 220 }}>
          <div className="subtle">Average per month</div>
          <div style={{ fontSize: 26, fontWeight: 700, marginTop: 6 }}>{formatCurrency(monthlyAverage)}</div>
        </div>
      </div>
                ...
                return (
                  <tr key={`${row.category}-${index}`} style={{ background }}>
                    ...
                    <td style={{ padding: "7px 10px", borderBottom: "1px solid #eee", color: "#555" }}>
                      {row.extraValue == null ? "" : numberFormatter.format(row.extraValue)}
                    </td>
                  </tr>
                );
              })}
      ...
      {subtotal && (
        <p className="subtle" style={{ marginTop: 12 }}>
          Sub total annual: {formatCurrency(subtotal.annualTotal)} · Overall annual: {formatCurrency(total?.annualTotal ?? 0)}
        </p>
      )}
    </div>
  );
